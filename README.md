> **Note:** This is a personal local copy of the original [DART repository](https://github.com/mehmetkeremturkcan/DART) by [Mehmet Kerem Turkcan](https://github.com/mehmetkeremturkcan). All credit for the research and implementation goes to the original author. I have made no original contributions to the model, training, or core architecture.
>
> **My changes** are limited to getting the project running on my local Windows 11 + RTX 4080 setup, plus minor code quality fixes (duplicate log handlers, narrower exception handling, input validation, `NotImplementedError` in place of bare `assert`/`RuntimeError` TODOs).
