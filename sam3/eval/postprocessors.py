...
class PostProcessImage(nn.Module):
        ...
        if ret_tensordict:
            ...
            if self.detection_threshold > 0.0:
                raise NotImplementedError(
                    "detection_threshold filtering is not supported with ret_tensordict=True"
                )
            try:
    ...
    def _process_masks(self, target_sizes, pred_masks, consistent=True, keep=None):
        ...
        if consistent:
            if keep is not None:
                raise NotImplementedError(
                    "keep-based mask filtering is not supported in consistent batch mode"
                )
            ...
            if self.convert_mask_to_rle:
                raise NotImplementedError(
                    "RLE mask conversion is not supported in consistent batch mode"
                )
            if self.to_cpu:
