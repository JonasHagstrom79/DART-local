        ...
        if text_cache is not None:
            ...
            if os.path.exists(text_cache):
                ...
                if data.get("class_names") == list(class_names):
                    ...
                    return
                cached = data.get("class_names", [])
                if isinstance(self.model, _TRTModelStub):
                    raise ValueError(
                        f"Text cache '{text_cache}' was built for classes {cached} "
                        f"but {list(class_names)} were requested. "
                        "Delete the cache file or run without --text-cache to recompute."
                    )
