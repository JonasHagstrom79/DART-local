...
def main():
    ...
    if args.mask_blocks:
        ...
        if args.efficient_backbone:
            ...
            sys.exit(1)
        for spec in mask_blocks:
            parts = spec.split(":")
            if len(parts) != 2 or not parts[0].isdigit() or parts[1] not in ("attn", "mlp"):
                print(
                    f"ERROR: Invalid --mask-blocks spec '{spec}'. "
                    "Expected comma-separated 'idx:type' pairs where type is "
                    "'attn' or 'mlp' (e.g. '25:attn,28:mlp')."
                )
                sys.exit(1)
